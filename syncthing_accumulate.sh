...
echo "----------------------------------------"
echo "----------------------------------------" >> "$LOG_FILE"
echo "$(date): Starting syncthing file accumulation" | tee -a "$LOG_FILE"

# Show directory sizes
SOURCE_SIZE=$(du -sh "$SOURCE_DIR" | cut -f1)
ACCUM_SIZE=$(du -sh "$ACCUMULATOR_DIR" | cut -f1)
echo "Source directory size: $SOURCE_SIZE" | tee -a "$LOG_FILE"
echo "Accumulator directory size before: $ACCUM_SIZE" | tee -a "$LOG_FILE"

# Perform rsync with detailed logging
echo "Starting rsync operation..."
rsync -avu --ignore-existing --progress "$SOURCE_DIR/" "$ACCUMULATOR_DIR/" 2>&1 | tee -a "$LOG_FILE"

# Log completion and final size
FINAL_SIZE=$(du -sh "$ACCUMULATOR_DIR" | cut -f1)
echo "Accumulator directory size after: $FINAL_SIZE" | tee -a "$LOG_FILE"
echo "$(date): Syncthing accumulation completed" | tee -a "$LOG_FILE"
echo "----------------------------------------" | tee -a "$LOG_FILE"
